...
class UtilityMixin:
    ...
    async def get_token_expiry(self, token: str) -> datetime:
        ...
        try:
            ...
            raise SupabaseAuthenticationError("Failed to decode token", original_error=e)
    
    @log_method()
    async def is_token_expiring_soon(self, token: str, threshold_minutes: int = 5) -> bool:
    ...
    async def health_check(self) -> bool:
        ...
        try:
            await self.supabase.from_("health_check").select("count", count="exact").execute()
            return True
        except Exception:
            return False 